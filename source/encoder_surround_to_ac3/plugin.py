...
from encoder_surround_to_ac3.lib.ffmpeg import StreamMapper, Probe, Parser

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.encoder_surround_to_ac3")
...
class PluginStreamMapper(StreamMapper):
    ...
    def calculate_bitrate(stream_info: dict):
        ...
        if int(channels) <= 2:
            logger.debug("Stream 'channels' is <= 2. Setting AC3 bit rate to 224k.")
            return '224'
        elif int(channels) <= 4:
    ...
    def test_stream_needs_processing(self, stream_info: dict):
        # Only process tracks with surround sound (more than 2 channels)
